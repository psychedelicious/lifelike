...
export const useCanvas = () => {
  ...
  const drawCells = React.useCallback(
    ({
      canvasBaseLayer,
      deadCellColor,
      aliveCellColor,
  ...
  const clearCanvas = React.useCallback(({ canvas }) => {
    const context = canvas.getContext('2d');
    context.clearRect(0, 0, canvas.width, canvas.height);
  }, []);

  const drawGridlines = React.useCallback(
    ({ canvasBaseLayer, gridlineColor, width, height, px }) => {
  ...
  const saveCanvasAsImage = React.useCallback(
  ...
  const changeCanvasSize = React.useCallback(
    ({
      canvasBaseLayerRef,
      canvasGridLayerRef,
      canvasDrawLayerRef,
